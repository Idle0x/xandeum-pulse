...
export default function ComparePage() {
  ...
  const benchmarks = useMemo(() => {
      ...
      return {
          network: { health: netHealthRaw.toString(), uptime: formatUptimePrecise(netUptimeRaw), storage: formatBytes(netStorageRaw), credits: netCreditsRaw.toLocaleString(), version: 'v1.2.0' },
  ...
  const removeNode = (pubkey: string) => { if (hoveredNodeKey === pubkey) setHoveredNodeKey(null); const k = selectedKeys.filter(x => x !== pubkey); setSelectedKeys(k); updateUrl(k); };
  const clearAllNodes = () => { setSelectedKeys([]); updateUrl([]); setHoveredNodeKey(null); };

  const toggleLeaderMetric = (metric: string) => {
      if (activeLeaderMetrics.includes(metric)) setActiveLeaderMetrics(prev => prev.filter(m => m !== metric));
      else setActiveLeaderMetrics(prev => [...prev, metric]);
  };
  ...
  useEffect(() => {
    if (printRef.current) {
        if (activeLeaderMetrics.length > prevLeaderCount.current) printRef.current.scrollTo({ left: 0, behavior: 'smooth' });
        else if (selectedKeys.length > prevSelectedCount.current) { const maxScroll = printRef.current.scrollWidth - printRef.current.clientWidth; printRef.current.scrollTo({ left: Math.max(0, maxScroll - 70), behavior: 'smooth' }); }
    }
    prevSelectedCount.current = selectedKeys.length; prevLeaderCount.current = activeLeaderMetrics.length;
  ...
  useEffect(() => {
    ...
    const { nodes: nodesParam } = router.query;
    if (nodesParam && typeof nodesParam === 'string') { setSelectedKeys(nodesParam.split(',')); setShowNetwork(true); hasProcessedDeepLink.current = true; }
  ...
  const handleExport = async (type: 'TABLE' | 'FULL') => {
      ...
      if (targetRef.current) {
          try {
              ...
              const dataUrl = await toPng(targetRef.current, { cacheBust: true, pixelRatio: 2, backgroundColor: '#020202', });
          ...
          } catch (err) { setToast('Export Failed'); setTimeout(() => setToast(null), 2000); }
      }
  };
  ...
  return (
    <div className="min-h-screen bg-[#020202] text-zinc-100 font-sans selection:bg-blue-500/30 flex flex-col overflow-hidden relative">
      ...
      <div style={{ position: 'absolute', top: '-10000px', left: '-10000px', opacity: 0, pointerEvents: 'none' }}>
          <div style={{ display: 'block' }}><PulseExportCanvas ref={exportTableRef} mode="TABLE" nodes={selectedNodes} leaders={leaderColumns} benchmarks={benchmarks} showNetwork={showNetwork} networkScope={networkScope} currentWinners={currentWinners} overallWinnerKey={overallWinnerKey} /></div>
                ...
                {isLeaderDropdownOpen && (
                    <div className="absolute top-full left-0 mt-2 w-40 bg-[#09090b] border border-zinc-800 rounded-xl shadow-xl overflow-hidden z-[70] flex flex-col">
                        {['STORAGE', 'CREDITS', 'HEALTH', 'UPTIME'].map(opt => {
                         ...
                         <button onClick={() => { setActiveLeaderMetrics([]); setIsLeaderDropdownOpen(false); }} className="px-4 py-3 text-[9px] font-bold text-left text-zinc-600 hover:text-red-400 border-t border-zinc-800 uppercase hover:bg-zinc-900 transition">None (Reset)</button>
                    </div>
                )}
            </div>
                    ...
                    <div className="absolute top-full left-0 mt-2 w-40 bg-[#09090b] border border-zinc-800 rounded-xl shadow-xl overflow-hidden z-[70] flex flex-col max-h-64 overflow-y-auto">
                        {watchlistNodes.length > 0 ? watchlistNodes.map(n => (
                            <div key={n.pubkey} className="flex justify-between items-center group/item hover:bg-zinc-800 transition border-b border-white/5 last:border-0">
                                <button onClick={() => addNode(n.pubkey!)} disabled={selectedKeys.includes(n.pubkey!)} className={`px-3 py-2 text-left flex-1 ${selectedKeys.includes(n.pubkey!) ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
            ...
            <div className="relative" ref={exportRef}>
                ...
                {isExportDropdownOpen && (
                    <div className="absolute top-full right-0 mt-2 w-44 bg-[#09090b] border border-zinc-800 rounded-xl shadow-xl overflow-hidden z-[70] flex flex-col">
                        <button onClick={() => handleExport('TABLE')} className="px-4 py-3 text-[10px] font-bold text-left text-zinc-300 hover:text-white hover:bg-zinc-800 transition uppercase flex items-center gap-2"><Table size={12} className="text-zinc-500" /> Table Only</button>
                        <button onClick={() => handleExport('FULL')} className="px-4 py-3 text-[10px] font-bold text-left text-zinc-300 hover:text-white hover:bg-zinc-800 transition uppercase flex items-center gap-2 border-t border-white/5"><BarChart size={12} className="text-zinc-500" /> With Charts</button>
                    </div>
                )}
            </div>
        </div>
         ...
         <div className="flex-initial min-h-[400px] flex flex-col bg-[#09090b]/60 backdrop-blur-2xl rounded-xl border border-white/5 shadow-2xl overflow-hidden relative mb-6">
             {selectedNodes.length === 0 && activeLeaderMetrics.length === 0 ? (
                 <div className="flex-1 flex flex-col items-center justify-center p-8 text-center relative overflow-hidden h-[400px]">
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-[0.03] scale-[2]"><div className="animate-[spin_60s_linear_infinite]"><Grid size={300} /></div></div>
                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none"><div className="w-96 h-96 bg-cyan-500/5 rounded-full animate-[ping_4s_cubic-bezier(0,0,0.2,1)_infinite]"></div></div>
                    <div className="relative z-10 flex flex-col items-center gap-6">
                        <button onClick={() => setIsSearchOpen(true)} className="px-10 py-4 bg-cyan-500/10 border border-cyan-500/20 text-cyan-50 hover:bg-cyan-500/20 font-black uppercase tracking-widest text-xs rounded-full shadow-[0_0_30px_rgba(34,211,238,0.1)] transition-all hover:scale-105 flex items-center gap-2 mb-2"><Plus size={16} className="animate-pulse" /> Add Node</button>
                        <div className="flex flex-col items-center gap-1"><h2 className="text-lg md:text-xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-500 uppercase tracking-[0.2em]">Compare Nodes</h2><p className="text-[10px] md:text-xs text-zinc-500 font-mono tracking-wide max-w-xs text-center leading-relaxed">Select a minimum of 1 node for head-to-head analysis against the network or top performers.</p></div>
                 ...
                 <main ref={printRef} onClick={() => setHoveredNodeKey(null)} className="flex-1 overflow-x-auto overflow-y-auto bg-transparent relative flex custom-scrollbar snap-x items-start content-start">
                    ...
                    {leaderColumns.map((leader, index) => {
                         const isWinner = { health: (leader.node.health || 0) === currentWinners.health, storage: (leader.node.storage_committed || 0) === currentWinners.storage, credits: (leader.node.credits || 0) === currentWinners.credits };
                        return (
                            <NodeColumn 
                    ...
                    {selectedNodes.map((node, index) => {
                        const isWinner = { health: (node.health || 0) === currentWinners.health, storage: (node.storage_committed || 0) === currentWinners.storage, credits: (node.credits || 0) === currentWinners.credits };
                        return (
                            <NodeColumn 
                                key={node.pubkey} 
                                node={node} 
          ...
          <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4" onClick={() => setIsSearchOpen(false)}>
              <div className="w-full max-w-xl bg-[#09090b] border border-zinc-800 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
                  <div className="p-6 border-b border-zinc-800 flex flex-col gap-4 bg-zinc-900/30">
                      <div className="flex items-center justify-between"><h3 className="text-sm font-black text-white uppercase tracking-widest">Add Source ({networkScope})</h3><button onClick={() => setIsSearchOpen(false)}><X size={16} className="text-zinc-500 hover:text-white" /></button></div>
                      <div className="flex items-center gap-3 bg-black/50 border border-zinc-800 rounded-xl px-4 py-3 focus-within:border-zinc-600 transition-colors"><Search size={16} className="text-zinc-500" /><input autoFocus type="text" placeholder="Search..." className="bg-transparent w-full text-xs text-white outline-none placeholder-zinc-600 font-mono" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
                  </div>
                  <div className="max-h-[400px] overflow-y-auto p-3 custom-scrollbar">
                      {availableNodes.filter(n => !searchQuery || n.pubkey?.toLowerCase().includes(searchQuery.toLowerCase()) || getSafeIp(n).includes(searchQuery)).map(node => (
                          <button key={node.pubkey} onClick={() => addNode(node.pubkey!)} disabled={selectedKeys.includes(node.pubkey!)} className={`w-full text-left p-3 rounded-xl flex justify-between items-center group transition mb-1 border border-transparent ${selectedKeys.includes(node.pubkey!) ? 'opacity-50 cursor-not-allowed bg-zinc-900/30' : 'hover:bg-zinc-900/80 hover:border-zinc-800 cursor-pointer'}`}>
                              <div><div className="text-xs font-bold text-zinc-200 group-hover:text-white font-mono flex items-center gap-3">{getSafeIp(node)}<span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${node.network === 'MAINNET' ? 'text-green-500 bg-green-500/10' : 'text-blue-500 bg-blue-500/10'}`}>{node.network}</span></div><div className="text-[10px] font-mono text-zinc-500 mt-0.5">{node.pubkey?.slice(0, 24)}...</div></div>
