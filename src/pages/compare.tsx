...
const ComparativeDelta = ({ val, base, type = 'number', reverse = false }: { val: number; base: number; type?: 'number' | 'bytes' | 'percent'; reverse?: boolean }) => {
  ...
  return (
    <div className={`text-[10px] md:text-xs font-mono font-bold mt-1 ${isGood ? 'text-cyan-400' : 'text-pink-500'}`}>
...
const MicroBar = ({ val, max, color }: { val: number, max: number, color: string }) => (
  <div className="w-12 md:w-20 h-1 bg-zinc-800 rounded-full overflow-hidden ml-auto">
...
const SectionHeader = ({ label, icon: Icon }: { label: string, icon: any }) => (
  <div className="h-6 md:h-8 bg-black/90 backdrop-blur-md border-y border-white/5 flex items-center px-3 gap-2 sticky left-0 z-10 w-full mt-1">
    <Icon size={12} className="text-zinc-500" />
    <span className="text-[9px] md:text-[10px] font-bold text-zinc-500 uppercase tracking-[0.1em]">{label}</span>
  </div>
);
...
const UnifiedLegend = ({ nodes, themes, metricMode = 'COUNTRY', specificMetric }: { nodes: Node[], themes: any[], metricMode: 'COUNTRY' | 'METRIC', specificMetric?: 'storage' | 'credits' | 'health' | 'uptime' }) => {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-6 mt-auto border-t border-white/5 bg-black/40 min-h-[100px]">
         {nodes.map((node, i) => {
             let metricDisplay = null;
             if (metricMode === 'COUNTRY') {
                 metricDisplay = (
                    <div className="flex items-center gap-2 mt-1">
                        {node.location?.countryCode && <img src={`https://flagcdn.com/w40/${node.location.countryCode.toLowerCase()}.png`} className="w-4 h-3 rounded-[1px]" />}
                        <span className="text-[10px] text-zinc-400 font-bold uppercase tracking-wide truncate">{node.location?.countryName || 'Unknown Region'}</span>
                    </div>
                 );
                 ...
                 metricDisplay = (
                    <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-[10px] text-zinc-500 uppercase tracking-wide">Actual</span>
                    </div>
                 );
             }
 
             return (
                 <div key={node.pubkey} className="flex gap-3 items-start p-3 rounded-lg border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] transition">
                     <div className="w-3 h-3 rounded-full mt-1 shrink-0 shadow-lg" style={{ backgroundColor: themes[i].hex }}></div>
                     <div className="flex flex-col gap-1 overflow-hidden">
                         <div className="text-[9px] font-mono text-zinc-500 truncate w-full tracking-wider uppercase" title={node.pubkey}>{node.pubkey}</div>
                         <div className="text-sm font-mono font-bold text-zinc-200">{getSafeIp(node)}</div>
                         {metricDisplay}
                     </div>
                 </div>
             )
...
const ControlRail = ({ showNetwork, leaderMetric, benchmarks }: any) => {
  const Benchmark = ({ label, val, subVal }: { label: string, val: string, subVal?: string }) => (
    <div className="flex flex-col justify-center h-[48px] md:h-[72px] border-b border-white/5 px-3 md:px-4">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center">
        <span className="text-[9px] md:text-xs text-zinc-500 font-bold uppercase tracking-wide">{label}</span>
        <span className="text-[9px] md:text-xs text-zinc-300 font-mono mt-0.5 md:mt-0">{showNetwork ? val : '-'}</span>
      </div>
      {leaderMetric && subVal && (
          <div className="flex items-center gap-1.5 mt-1 text-yellow-600/80">
              <span className="text-[8px] uppercase font-bold border border-yellow-900/40 px-1 rounded bg-yellow-900/10">TOP</span>
              <span className="text-[9px] font-mono">{subVal}</span>
          </div>
      )}
    </div>
  );
  ...
  return (
    <div className="sticky left-0 z-40 bg-[#09090b] border-r border-white/10 w-[110px] md:w-[200px] shrink-0 flex flex-col shadow-[4px_0_24px_-4px_rgba(0,0,0,0.8)] rounded-tl-xl rounded-bl-xl">
      <div className="h-28 md:h-32 border-b border-white/5 p-4 flex flex-col justify-end bg-black rounded-tl-xl">
        <div className="text-[10px] md:text-xs font-black text-zinc-500 uppercase tracking-widest flex items-center gap-2"><Settings2 size={12} /> METRICS</div>
      </div>
      ...
      <div className="bg-transparent">
        ...
        <div className="h-[32px] border-t border-white/5 bg-black/50 rounded-bl-xl"></div>
      </div>
    </div>
  );
};
...
const NodeColumn = ({ node, onRemove, anchorNode, theme, winners, overallWinner, benchmarks, leaderMetric, showNetwork }: any) => {
  const Row = ({ children }: { children: React.ReactNode }) => (<div className={`h-[48px] md:h-[72px] flex flex-col justify-center px-4 min-w-[140px] md:min-w-[240px] relative border-b border-white/5`}>{children}</div>);
  const SectionSpacer = () => <div className="h-6 md:h-8 bg-transparent border-y border-transparent mt-1"></div>;
  ...
  return (
    <div className={`flex flex-col min-w-[140px] md:min-w-[240px] ${theme.bodyBg} relative border-r border-white/5 last:rounded-tr-xl last:rounded-br-xl`}>
      {/* Header */}
      <div className={`h-28 md:h-32 ${theme.headerBg} p-4 flex flex-col items-center justify-center relative overflow-hidden group first:rounded-tr-xl`}>
        {overallWinner && <div className="absolute top-2 left-2 animate-in zoom-in duration-500"><Crown size={14} className="text-yellow-400 fill-yellow-400 drop-shadow-sm" /></div>}
        
        <div className="flex items-center gap-3 mb-2">
            {node.location?.countryCode && <img src={`https://flagcdn.com/w40/${node.location.countryCode.toLowerCase()}.png`} className="w-5 h-3.5 rounded-[2px] shadow-sm" />}
            <div className={`text-xs md:text-sm font-bold font-mono text-white`}>{getSafeIp(node)}</div>
        </div>
        
        <div className="text-[10px] md:text-xs text-white/60 font-mono bg-black/20 px-2 py-1 rounded border border-white/10">{node.pubkey?.slice(0, 16)}...</div>
      </div>

      <div className="relative z-10">
        <SectionSpacer />
        <Row><span className={`text-xs md:text-sm font-mono font-medium ${node.version === '0.0.0' ? 'text-zinc-600' : 'text-zinc-300'}`}>{node.version}</span></Row>
        <Row><span className={`text-[10px] md:text-xs px-2 py-1 rounded border w-fit font-bold ${node.network === 'MAINNET' ? 'text-green-400 border-green-500/20 bg-green-500/5' : 'text-blue-400 border-blue-500/20 bg-blue-500/5'}`}>{node.network}</span></Row>

        <SectionSpacer />
        <Row>
            <div className="flex flex-col w-full">
                <div className="flex justify-between items-center w-full">
                    <span className="text-xs md:text-base font-bold font-mono text-zinc-200">{node.health}</span>
                    {winners.health && <CheckCircle size={14} className="text-green-500" />}
                </div>
                {(() => { const ctx = getContext('health'); return ctx.base > 0 && <ComparativeDelta val={node.health} base={ctx.base} />; })()}
            </div>
            <div className="absolute bottom-3 right-4 opacity-50"><MicroBar val={node.health} max={100} color={node.health >= 90 ? '#22c55e' : '#eab308'} /></div>
        </Row>
        <Row><span className="text-xs md:text-base font-mono text-zinc-300">{formatUptimePrecise(node.uptime)}</span></Row>

        <SectionSpacer />
        <Row>
             <div className="flex flex-col w-full">
                <div className="flex justify-between items-center w-full">
                    <span className="text-xs md:text-base font-bold font-mono text-zinc-200">{formatBytes(node.storage_committed)}</span>
                    {winners.storage && <CheckCircle size={14} className="text-green-500" />}
                </div>
        ...
        <Row>
            <div className="flex flex-col w-full">
                <div className="flex justify-between items-center w-full">
                    <span className="text-xs md:text-base font-mono text-zinc-300">{formatBytes(node.storage_used)}</span>
                    <span className="text-[10px] text-zinc-500 font-mono mt-0.5">{node.storage_committed > 0 ? ((node.storage_used / node.storage_committed) * 100).toFixed(0) : 0}% Utilized</span>
                </div>
            </div>
            <div className="absolute bottom-3 right-4 opacity-50"><MicroBar val={node.storage_used} max={node.storage_committed} color="#60a5fa" /></div>
        </Row>
        ...
        <Row>
            <div className="flex flex-col w-full">
                <div className="flex justify-between items-center w-full">
                    <span className="text-xs md:text-base font-bold font-mono text-zinc-200">{node.credits?.toLocaleString()}</span>
                    {winners.credits && <CheckCircle size={14} className="text-green-500" />}
                </div>
                {(() => { const ctx = getContext('credits'); return ctx.base > 0 && <ComparativeDelta val={node.credits} base={ctx.base} />; })()}
            </div>
        </Row>
        <Row><span className="text-xs md:text-base font-mono text-zinc-500">#{node.rank || '-'}</span></Row>

        {/* TRASHCAN ROW */}
        <div className="h-[32px] border-t border-white/5 flex items-center justify-center bg-black/20 hover:bg-red-500/10 transition-colors cursor-pointer group/trash" onClick={onRemove}>
            <Trash2 size={12} className="text-zinc-700 group-hover/trash:text-red-500 transition-colors" />
        </div>
      </div>
    </div>
  );
...
const OverviewLegend = ({ nodes, themes }: { nodes: Node[], themes: any[] }) => {
    return (
      <div className="flex flex-wrap items-center justify-center gap-6 p-4 border-t border-white/5 bg-black/40 min-h-[60px]">
          {nodes.map((node, i) => (
              <div key={node.pubkey} className="flex items-center gap-2">
                   <div className="w-3 h-3 rounded-full shadow-[0_0_8px_rgba(0,0,0,0.5)] border border-white/10" style={{ backgroundColor: themes[i].hex }}></div>
                   <span className="text-[10px] md:text-xs font-mono font-bold text-zinc-300">{getSafeIp(node)}</span>
              </div>
          ))}
      </div>
    )
...
const InterpretationPanel = ({ contextText, title = "ANALYST EVALUATION" }: { contextText: string, title?: string }) => (
    <div className="px-6 py-4 bg-zinc-900/30 border-t border-white/5 flex items-start gap-4">
        <Info size={16} className="text-blue-400 shrink-0 mt-0.5" />
        <div className="flex flex-col gap-1">
            <h4 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{title}</h4>
            <p className="text-sm text-zinc-300 leading-relaxed max-w-4xl">{contextText}</p>
        </div>
    </div>
);
...
const SynthesisEngine = ({ nodes, themes, networkScope }: { nodes: Node[], themes: typeof PLAYER_THEMES, networkScope: string }) => {
  ...
  const ChartCell = ({ title, icon: Icon, children }: any) => (
    <div className="bg-black/20 border border-white/5 rounded-xl p-6 flex flex-col items-center justify-end relative overflow-hidden group hover:border-white/10 transition-colors">
        ...
        <div className="flex items-end justify-center gap-4 md:gap-6 h-32 md:h-48 w-full px-4">
            {children}
        </div>
    </div>
  );
  ...
  return (
    <div className="shrink-0 min-h-[600px] border border-white/5 bg-[#09090b]/40 backdrop-blur-xl flex flex-col relative z-40 rounded-xl mt-6 shadow-2xl overflow-hidden">
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-50">
            <div className="bg-zinc-900/90 backdrop-blur-md p-1.5 rounded-full flex gap-2 border border-white/10 shadow-2xl">
                {[ { id: 'OVERVIEW', icon: BarChart3, label: 'Overview' }, { id: 'MARKET', icon: PieChart, label: 'Market Share' }, { id: 'TOPOLOGY', icon: MapIcon, label: 'Topology' } ].map((t) => (
                    <button key={t.id} onClick={() => setTab(t.id as any)} className={`px-6 py-2 rounded-full text-[10px] md:text-xs font-bold uppercase transition-all duration-300 flex items-center gap-2 ${tab === t.id ? 'bg-zinc-100 text-black shadow-[0_0_20px_rgba(255,255,255,0.2)]' : 'bg-white/5 text-zinc-500 hover:text-zinc-300 hover:bg-white/10'}`}>
        ...
        <div className="flex-1 overflow-hidden relative flex flex-col pt-24">
            {tab === 'OVERVIEW' && (
                <>
                <div className="grid grid-cols-2 grid-rows-2 gap-4 md:gap-6 p-6 h-full">
                    <ChartCell title="Storage Capacity" icon={Database}>
                        {nodes.map((n, i) => (
                             <div key={n.pubkey} className="w-8 md:w-12 bg-zinc-800/30 rounded-t-sm relative group/bar h-full flex flex-col justify-end">
                                 ...
                                 <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-bold font-mono text-zinc-400 opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap bg-black/80 px-2 py-1 rounded border border-white/10 z-10">{formatBytes(n.storage_committed || 0)}</div>
                             </div>
                        ))}
                    </ChartCell>
                    <ChartCell title="Credits Earned" icon={Zap}>
                        {nodes.map((n, i) => (
                             <div key={n.pubkey} className="w-8 md:w-12 bg-zinc-800/30 rounded-t-sm relative group/bar h-full flex flex-col justify-end">
                                 ...
                                 <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-bold font-mono text-zinc-400 opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap bg-black/80 px-2 py-1 rounded border border-white/10 z-10">{n.credits?.toLocaleString()}</div>
                             </div>
                        ))}
                    </ChartCell>
                    <ChartCell title="Health Score" icon={Activity}>
                        {nodes.map((n, i) => (
                             <div key={n.pubkey} className="w-8 md:w-12 bg-zinc-800/30 rounded-t-sm relative group/bar h-full flex flex-col justify-end">
                                 ...
                                 <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-bold font-mono text-zinc-400 opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap bg-black/80 px-2 py-1 rounded border border-white/10 z-10">{n.health}/100</div>
                             </div>
                        ))}
                    </ChartCell>
                     <ChartCell title="Uptime Duration" icon={Clock}>
                        {nodes.map((n, i) => (
                             <div key={n.pubkey} className="w-8 md:w-12 bg-zinc-800/30 rounded-t-sm relative group/bar h-full flex flex-col justify-end">
                                 ...
                                 <div className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-[10px] font-bold font-mono text-zinc-400 opacity-0 group-hover/bar:opacity-100 transition-opacity whitespace-nowrap bg-black/80 px-2 py-1 rounded border border-white/10 z-10">{formatUptimePrecise(n.uptime || 0)}</div>
                             </div>
                        ))}
                    </ChartCell>
                </div>
                ...
                <>
                    <div className="relative flex flex-col flex-1">
                        ...
                        <div className="absolute top-0 right-8 z-20">
                            <button onClick={() => setIsMetricDropdownOpen(!isMetricDropdownOpen)} className="flex items-center gap-2 px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-white border border-zinc-600 rounded-lg text-xs font-bold uppercase transition">
                                <span className="opacity-50">Analyzing:</span> {marketMetric} <ChevronDown size={14} />
                            </button>
                        ...
                        <div className="flex-1 flex items-center justify-center p-8">
                            {marketMetric !== 'health' ? (
                                <div className="flex flex-col items-center gap-6 animate-in zoom-in-50 duration-500">
                                    <div className="w-72 h-72 rounded-full relative flex items-center justify-center shadow-[0_0_50px_rgba(0,0,0,0.5)] transition-all" style={{ background: getConicGradient(marketMetric as any) }}>
                                        <div className="w-56 h-56 bg-[#050505] rounded-full flex flex-col items-center justify-center z-10 shadow-inner border border-white/5 p-4 text-center">
                                            {marketMetric === 'storage' && <Database size={32} className="text-zinc-600 mb-2" />}
                                            {marketMetric === 'credits' && <Zap size={32} className="text-zinc-600 mb-2" />}
                                            {marketMetric === 'uptime' && <Clock size={32} className="text-zinc-600 mb-2" />}
                                            
                                            <span className="text-sm font-bold text-zinc-400 tracking-widest uppercase mb-1">{marketMetric} Share</span>
                    ...
                    <InterpretationPanel contextText={getEvaluation('MARKET', marketMetric)} title={`${marketMetric} EVALUATION`} />
                </>
            )}

            {tab === 'TOPOLOGY' && (
                <div className="flex flex-col h-full">
                    <div className="flex-1 rounded-xl overflow-hidden border border-white/5 bg-[#050505] mx-6 relative group shadow-inner">
                            ...
                            <ZoomableGroup zoom={pos.zoom} center={pos.coordinates as [number, number]} maxZoom={10}>
                                ...
                                {nodes.map((n, i) => (n.location && (
                                    <Marker key={n.pubkey} coordinates={[n.location.lon, n.location.lat]}>
                                        <circle r={12/pos.zoom} fill={themes[i].hex} fillOpacity={0.4} className="animate-pulse" />
                                        <circle r={6/pos.zoom} fill="#fff" stroke={themes[i].hex} strokeWidth={3/pos.zoom} />
                                    </Marker>
                                )))}
                            </ZoomableGroup>
                        </ComposableMap>
...
const EmptySlot = ({ onClick }: { onClick: () => void }) => (
  <div className="flex flex-col min-w-[140px] md:min-w-[240px] h-full bg-white/[0.01] group cursor-pointer hover:bg-white/[0.03] transition relative last:rounded-tr-xl last:rounded-br-xl" onClick={onClick}>
    <div className="h-28 md:h-32 p-2 flex flex-col items-center justify-center border-b border-white/5 bg-black/40 first:rounded-tr-xl">
      <Plus size={16} className="text-zinc-700 group-hover:text-zinc-400 transition" />
    </div>
    ...
    <div className="min-h-screen bg-[#020202] text-zinc-100 font-sans selection:bg-blue-500/30 flex flex-col overflow-hidden relative">
      ...
      <header className="shrink-0 pt-6 pb-4 px-4 md:px-8 relative z-50 flex justify-center">
        ...
        <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-xs font-mono text-zinc-500 uppercase tracking-widest">Selected Nodes: {selectedNodes.length}</span>
        </div>
      </header>
      ...
      {isSearchOpen && (
          <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-4">
              <div className="w-full max-w-xl bg-[#09090b] border border-zinc-800 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
