...
export const NodeCard = ({ 
  ...
  // --- 1. LOCAL CYCLE LOGIC (Moved back inside for reliability) ---
  ...
  // --- 2. SINGLE RESPONSIVE CARD (Solves Duplication) ---
  return (
    <div
    ...
    >
        ...
        <div className="overflow-hidden pr-2 w-full">
          ...
          <div className="relative h-4 md:h-6 w-full">
             <div className="absolute inset-0 transition-opacity duration-300 group-hover:opacity-0 flex items-center">
                <span className="font-mono text-[10px] md:text-sm text-zinc-300 truncate w-full">
      ...
      <div className="space-y-1.5 md:space-y-3">
        <div className="flex justify-between items-center text-[9px] md:text-xs">
          ...
          <span className={`px-1.5 py-0.5 md:px-2 rounded transition-all duration-500 ${
        ...
        <div className="pt-1 md:pt-2">
          ...
          <div className={`flex justify-between items-center text-[9px] md:text-xs p-1 md:p-2 rounded-lg border transition-colors ${
            ...
            {(node as any).isUntracked ? (
              <div className="flex items-center gap-1 md:gap-2 text-zinc-500 w-full justify-center font-bold tracking-wide">
                <AlertTriangle size={8} className="md:w-[10px] md:h-[10px]"/> <span className="truncate">NO STORAGE CREDITS</span>
              </div>
            ) : node.credits !== null ? (
              <>
                <div className="flex items-center gap-1 md:gap-1.5">
                  <Medal size={8} className={`md:w-[10px] md:h-[10px] ${node.rank===1?'text-yellow-400':'text-zinc-500'}`} />
                  <span className="text-zinc-400 font-bold">#{node.rank}</span>
                </div>
                <div className="flex items-center gap-1 md:gap-1.5">
                  <span className="text-zinc-300 font-mono">
                    {node.credits >= 1000 && window.innerWidth < 768 ? (node.credits/1000).toFixed(1) + 'k' : node.credits.toLocaleString()}
                  </span>
                  <Wallet size={8} className="text-yellow-600 md:w-[10px] md:h-[10px]"/>
                </div>
              </>
            ) : (
              <div className="flex items-center gap-1 md:gap-2 text-red-400 w-full justify-center font-bold italic">
                <AlertOctagon size={8} className="md:w-[10px] md:h-[10px]"/> OFFLINE
              </div>
            )}
          </div>
        </div>
