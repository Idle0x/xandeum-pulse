...
export const StatsOverview = ({
  ...
  const healthData = history.map(p => ({ 
    date: p.date, 
    value: networkFilter === 'MAINNET' ? p.mainnet_avg_health : networkFilter === 'DEVNET' ? p.devnet_avg_health : p.avg_health 
  }));

  const consensusData = history.map(p => ({ 
    date: p.date, 
