    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[150] flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#09090b] border border-zinc-800 rounded-3xl p-6 md:p-8 max-w-2xl w-full shadow-2xl animate-in zoom-in-95 fade-in duration-200 overflow-y-auto max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4 md:p-5 relative overflow-hidden">
             ...
             <div className="grid grid-cols-2 gap-8 divide-x divide-zinc-800">
                <div className="text-center">
                   ...
                   <div className={`text-2xl md:text-3xl font-black tracking-tight ${activeTab === 'ALL' ? 'text-purple-400' : activeTab === 'MAINNET' ? 'text-white' : 'text-white'}`}>
          ...
          {/* --- ROW 2: SPLIT & BENCHMARKS --- */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
             
             {/* LEFT: NETWORK SPLIT (Only visible on ALL) */}
             {activeTab === 'ALL' ? (
                 <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-3 flex items-center gap-4">
                    ...
                    <div className="flex-1 space-y-1.5">
                       <div>
                          ...
                          <div className="text-xs font-mono text-white leading-none">{formatBytes(dashboardData.mainnetSum)}</div>
                       </div>
                       <div>
                          ...
                          <div className="text-xs font-mono text-white leading-none">{formatBytes(dashboardData.devnetSum)}</div>
                       </div>
                    </div>
                 </div>
             ) : (
                // If specific network, we can show a summary or just span the benchmarks. 
                // Using a placeholder context box to keep layout balanced or simply "Network Status"
                <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-4 flex flex-col justify-center">
                    <div className="text-[9px] text-zinc-500 uppercase font-bold mb-1 flex items-center gap-2"><Server size={10}/> Network Scope</div>
                    <div className="text-sm font-bold text-white">Viewing {activeTab} Only</div>
                    <div className="text-[9px] text-zinc-500 mt-1">Global statistics filtered to {activeTab.toLowerCase()}.</div>
                </div>
             )}

             {/* RIGHT: BENCHMARKS (Single Box) */}
             <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-4 flex flex-col justify-center">
