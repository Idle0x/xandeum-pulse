...
export const CapacityModal = ({ onClose, nodes }: CapacityModalProps) => {
  ...
  const dashboardData = useMemo(() => {
    ...
    let mainnetSum = 0; let devnetSum = 0;
    nodes.forEach(n => {
    ...
    return {
      totalCommitted: tCommitted, totalUsed: tUsed, nodeCount: filteredNodes.length,
      median, average, top10Sum, mainnetSum, devnetSum,
        ...
        {slices.map((slice, i) => {
          ...
          return (
            <circle
              ...
              // This transition handles the rotation and slice resizing
      ...
      <div className="bg-[#09090b] border border-zinc-800 rounded-3xl p-6 max-w-3xl w-full shadow-2xl animate-in zoom-in-95 duration-500 overflow-y-auto max-h-[90vh] custom-scrollbar" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-2">
            <div className="flex bg-zinc-900 p-1 rounded-lg border border-zinc-800">
              {(['ALL', 'MAINNET', 'DEVNET'] as const).map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-1.5 rounded-md text-[10px] font-bold transition-all duration-300 ${activeTab === tab ? 'bg-zinc-100 text-black' : 'text-zinc-500 hover:text-zinc-300'}`}>
                  {tab}
                </button>
              ))}
            </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            {/* Committed Card */}
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 h-24 flex flex-col justify-between relative overflow-hidden transition-colors">
              <div className="absolute inset-0 opacity-10 mt-6 px-2"><HistoryChart data={heroChartData} color={activeTab === 'ALL' ? '#a855f7' : activeTab === 'MAINNET' ? '#22c55e' : '#3b82f6'} loading={trendLoading} height={60} /></div>
              ...
              <div className={`text-3xl font-black tabular-nums tracking-tighter transition-colors duration-500 ${themeColor}`}>{formatBytes(dashboardData.totalCommitted)}</div>
            </div>
            {/* Load State Card */}
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-4 h-24 flex flex-col justify-between">
              ...
              <div className="text-3xl font-black text-blue-400 tabular-nums tracking-tighter">{formatBytes(dashboardData.totalUsed)}</div>
            </div>
          </div>

          <div className="h-60"><CapacityEvolutionChart history={evoHistory} loading={evoLoading} timeRange={timeRange} onTimeRangeChange={setTimeRange} capacityKey={capacityKey} usedKey={usedKey} /></div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {/* LEFT: FLUID HEALTH BLOCK */}
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-4 flex flex-col transition-all duration-500">
              <div className="flex justify-between items-center mb-4">
                <div className="flex flex-col">
                  <span className="text-[9px] text-zinc-500 uppercase font-bold flex items-center gap-1.5"><Users size={12} className={`transition-colors duration-500 ${themeColor}`}/> Active Nodes</span>
                  <span className="text-3xl font-black text-white tabular-nums tracking-tighter animate-in fade-in duration-500">{dashboardData.nodeCount}</span>
                </div>
              ...
              <div className="mt-auto pt-4 border-t border-zinc-800/50">
                <div className="text-[9px] text-zinc-400 font-black uppercase tracking-widest mb-3 flex items-center gap-2"><Scale size={12}/> Distribution & Benchmark</div>
                <div className="space-y-3">
                   <div className="flex justify-between text-[10px] font-mono">
                      <span className="text-zinc-500">Median</span>
                      <span className="text-white font-bold transition-all duration-300">{formatBytes(dashboardData.median)}</span>
                   </div>
                   {/* FLUID PROGRESS BAR */}
                   ...
                   <div className="flex justify-between text-[10px] font-mono"><span className="text-zinc-500">Average</span><span className="text-zinc-400">{formatBytes(dashboardData.average)}</span></div>
                </div>
              </div>
            </div>

            {/* RIGHT: FLUID WHALE DOMINANCE */}
            <div className="bg-yellow-500/5 border border-yellow-500/10 rounded-2xl p-5 flex items-center justify-between transition-colors duration-500">
              <div className="flex flex-col gap-1 max-w-[60%]">
                ...
                <div className="text-4xl font-black text-yellow-500 tracking-tighter my-1 animate-in fade-in duration-700">
                  {((dashboardData.top10Sum / (dashboardData.totalCommitted || 1)) * 100).toFixed(1)}%
                </div>
                <div className="text-[9px] leading-relaxed text-zinc-500 uppercase font-bold">Top 10 controls <span className="text-white">{formatBytes(dashboardData.top10Sum)}</span></div>
              </div>
              <div className="w-24 h-24 relative">
                {renderPie([{ value: dashboardData.top10Sum, color: '#eab308' }, { value: dashboardData.remainderSum, color: '#27272a' }])}
              </div>
            </div>
          </div>
        </div>
