...
interface NetworkPulseChartProps {
  ...
  // NEW: Dynamic keys for the new database schema
  healthKey: keyof NetworkHistoryPoint;
  countKey: keyof NetworkHistoryPoint;
}

export const NetworkPulseChart = ({ 
  history, loading, timeRange, onTimeRangeChange, healthKey, countKey 
}: NetworkPulseChartProps) => {
      ...
      return (
        <div className="px-2 py-1.5 rounded bg-zinc-950 border border-zinc-800 shadow-xl text-[10px] backdrop-blur-md">
          ...
          <div className="flex flex-col gap-0.5">
             <div className="font-bold font-mono text-green-400 flex items-center gap-1.5">
    ...
    <div className="w-full h-full flex flex-col rounded-xl border border-zinc-800 bg-black/40 p-3 relative">
      ...
      <div className="flex justify-between items-center mb-1 relative z-20 h-6 shrink-0">
         <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Network Pulse</span>
            <span className="text-[9px] text-zinc-600 hidden md:block">| Green: Health • Blue: Count</span>
         </div>
      ...
      <div className="flex-1 w-full min-h-0 relative">
         ...
         <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={history} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
               ...
               <YAxis yAxisId="left" domain={[0, 100]} axisLine={false} tickLine={false} tick={{fontSize: 8, fill: '#52525b'}} width={25} />
               <YAxis yAxisId="right" orientation="right" domain={['auto', 'auto']} axisLine={false} tickLine={false} tick={{fontSize: 8, fill: '#52525b'}} width={25} />
               <Tooltip content={<CustomTooltip />} cursor={{ stroke: '#3f3f46', strokeWidth: 1 }} />
               {/* DYNAMIC KEYS USED HERE */}
               <Area yAxisId="left" type="monotone" dataKey={healthKey} stroke="#22c55e" strokeWidth={1.5} fill="url(#healthGrad)" isAnimationActive={false} />
               <Line yAxisId="right" type="monotone" dataKey={countKey} stroke="#3b82f6" strokeWidth={1.5} dot={false} isAnimationActive={false} />
            </AreaChart>
         </ResponsiveContainer>
      </div>
    </div>
