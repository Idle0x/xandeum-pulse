...
interface StorageViewProps {
  ...
  historyLoading?: boolean; // Added to prevent badge flickering
}
...
export const StorageView = ({ 
  ...
  const chartData = useMemo(() => {
    return history.map(p => ({
      date: p.date,
      value: p.health // We use health/uptime as a proxy for "reliability" in this context
    }));
  }, [history]);

  // 2. Calculate Reliability Score from history prop
  // (Previously handled by hook, now handled locally from prop data)
  const reliabilityScore = useMemo(() => {
     if (!history || history.length === 0) return 0;
     const healthySnapshots = history.reduce((acc, p) => acc + (p.health >= 80 ? 1 : 0), 0);
     return Math.round((healthySnapshots / history.length) * 100);
  }, [history]);
  ...
  return (
    <div className={`h-full flex flex-col ${zenMode ? '' : 'animate-in fade-in slide-in-from-right-2 duration-300'}`}>
      ...
      <div className="flex-grow flex flex-col gap-4">
        ...
        {/* NETWORK COMPARISON CARD with SHADOW CHART */}
        <div className={`p-4 rounded-xl border text-center relative overflow-hidden group ${zenMode ? 'bg-black border-zinc-700' : 'bg-zinc-900/40 border-zinc-800/60'}`}>
          ...
          {/* RELIABILITY BADGE (Restored) */}
          {/* Only shows if loaded and excellent (>90%) */}
        ...
        {/* UTILIZATION DONUT & STATUS BADGE */}
