...
interface HealthViewProps {
  ...
  networkStats: any; // Or strictly type your NetworkStats interface
}
...
export const HealthView = ({ node, zenMode, onBack, avgNetworkHealth, medianStorage, networkStats }: HealthViewProps) => {
  ...
  // Note: If you don't pass totalNodes, we can default to 1 or pass it as prop. 
  // Assuming networkStats has totalNodes based on original code.
        ...
        <div className="space-y-5">
          {metrics.map((m) => {
            if (m.label === 'Reputation Score') {
             ...
             const barColor = (m.rawVal || 0) >= 80 ? 'bg-green-500' : (m.rawVal || 0) >= 50 ? 'bg-yellow-500' : 'bg-red-500';
             ...
             return (
               <div key={m.label}>
                 ...
                 <div className="h-2 bg-zinc-800 rounded-full relative">
                   <div className={`h-full rounded-full ${barColor}`} style={{ width: `${m.rawVal || 0}%` }}></div>
            ...
            return (
              <div key={m.label}>
                <div className="flex justify-between text-xs mb-2">
                  <span className="text-zinc-400 font-bold flex items-center gap-2">
                      {m.label}
                      {m.label === 'Storage Capacity' && node ? (
                          <span className="text-[9px] font-mono text-zinc-600 font-normal">
                              {getStorageBreakdownText(node, medianStorage)}
                          </span>
                      ) : (
                          <span className="text-[9px] font-mono text-zinc-600 font-normal">
                              (Base: {rawVal})
