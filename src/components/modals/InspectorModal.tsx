...
import { 
  ...
  ArrowUpRight, MapPin, AlertOctagon
} from 'lucide-react';
...
interface InspectorModalProps {
  ...
  onShowToast: (msg: string) => void; // ADDED: Required for notifications
}
...
export const InspectorModal = ({
  ...
  const handleLeaderboardNav = (e: React.MouseEvent) => {
    ...
    // Priority 1: Check if node is explicitly untracked
    if ((selectedNode as any).isUntracked) {
       onShowToast("This node is currently not receiving storage credits. It may be in a proving phase or below stake thresholds.");
       return;
    }

    // Priority 2: Check if API is globally offline
  ...
  // --- REPUTATION TIER LOGIC (RESTORED NO HYPOTHETICAL RANK) ---
  ...
  const shimmerAnimation = "animate-[shimmer_12s_infinite_linear]"; 
               ...
               {modalView !== 'overview' ? (
                 ...
                 <div className="flex flex-col gap-3 md:gap-4 h-full">
                    ...
                    <div className="grid grid-cols-2 gap-2 md:hidden">
                                ...
                                <div className="mt-auto flex flex-col gap-1.5">
                                    <div className="flex justify-between items-end">
                                        <div className="flex flex-col items-center">
                                            ...
                                            <div className="text-sm font-bold text-indigo-100 drop-shadow-md">{formatBytes(selectedNode.storage_used).split(' ')[0]}</div>
                                        </div>
                                        <div className="w-px h-6 bg-white/5 mx-1"></div>
                                        <div className="flex flex-col items-center">
                                            <div className="text-[8px] font-bold text-zinc-500 uppercase shadow-black drop-shadow-sm">Committed</div>
                                            <div className="text-sm font-bold text-indigo-100 drop-shadow-md">{formatBytes(selectedNode.storage_committed).split(' ')[0]}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        {/* ROW 3: REPUTATION (MOBILE) - RESTORED LOGIC */}
                        <div 
                            onClick={handleLeaderboardNav} 
                            className={`h-24 p-3 rounded-2xl relative overflow-hidden group cursor-pointer hover:-translate-y-0.5 transition-all duration-300 ring-1 ring-yellow-500/20 hover:ring-yellow-500/50 ${zenMode ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-900/80 border-yellow-900/30'}`}
                        >
                        ...
                        <Link 
                            ...
                            className={`h-24 p-3 rounded-2xl relative overflow-hidden block group cursor-pointer ring-1 ring-blue-500/20 hover:ring-blue-500/50 ${zenMode ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-900/80 border-blue-900/30'}`}
                        >
                            ...
                            <div className="flex justify-between items-start relative z-10 w-full">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[9px] font-bold uppercase text-zinc-500 leading-tight">PHYSICAL</span>
                                </div>
                            </div>
                            <div className="relative z-10 flex flex-col h-full justify-between pb-3 pt-2">
                                <div className="text-[10px] font-mono text-zinc-400 truncate w-full">{getSafeIp(selectedNode)}</div>
                    ...
                    <div className="hidden md:grid grid-cols-1 md:grid-cols-2 gap-4">
                       ...
                       {/* REPUTATION DEEP LINK (RESTORED LOGIC) */}
                       <div 
                          onClick={handleLeaderboardNav} 
                        ...
                        >
                          ...
                          <div className={`absolute bottom-2 right-4 text-[9px] font-bold uppercase ${(selectedNode as any).isUntracked ? 'text-zinc-600' : 'text-yellow-500 opacity-0 group-hover:opacity-100'} transition-opacity ${breatheAnimation}`}>OPEN LEADERBOARD</div>
                       </div>
