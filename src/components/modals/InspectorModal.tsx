                 ...
                 <div className="flex flex-col gap-3 md:gap-4 h-full">
                    ...
                    <div className="grid grid-cols-2 gap-2 md:hidden">
                        ...
                        <Link href={`/map?focus=${getSafeIp(selectedNode)}`} className={`h-24 p-3 rounded-2xl relative overflow-hidden block group cursor-pointer ring-1 ring-blue-500/20 hover:ring-blue-500/50 ${zenMode ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-900/80 border-blue-900/30'}`}>
                            ...
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0"><div className="relative flex h-1.5 w-1.5"><span className="animate-[slow-ping_3s_infinite] absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span><span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-blue-500 border border-white/20"></span></div></div>
                            ...
                            <div className="relative z-10 flex flex-col h-full justify-between pb-3 pt-2"><div className="text-[10px] font-mono text-zinc-400 truncate w-full">{getSafeIp(selectedNode)}</div><div className="flex items-center gap-2 text-[10px] font-bold text-white leading-none"><span className="text-sm leading-none">{getFlagEmoji(selectedNode.location?.countryCode)}</span><span>{selectedNode.location?.countryName || 'Unknown'}</span><MapPin size={8} className="text-blue-500/80 ml-auto" /></div></div>
                        </Link>
                    </div>
                    ...
                    <div className="hidden md:grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
                      ...
                      <div className={`rounded-2xl md:rounded-3xl p-4 md:p-6 flex flex-col justify-between relative overflow-hidden group cursor-pointer hover:-translate-y-1 transition-all duration-300 ring-1 ${zenMode ? 'bg-zinc-900' : 'bg-zinc-900/30'} ${healthRingColor}`} onClick={() => handleCardToggle('health')}>
                         ...
                         <div className="self-center hidden md:flex items-center justify-center relative z-10"><div className={`absolute inset-0 rounded-full blur-xl animate-[slow-pulse_12s_infinite_ease-in-out] ${healthGlowColor}`}></div><RadialProgress score={selectedNode.health || 0} size={140} /></div>
                    ...
                    <div className="hidden md:grid grid-cols-1 md:grid-cols-2 gap-4">
                       ...
                       <Link href={`/map?focus=${getSafeIp(selectedNode)}`}>
                         <div className={`h-40 p-5 rounded-2xl border group cursor-pointer relative overflow-hidden flex flex-col justify-between hover:-translate-y-0.5 transition-all duration-300 ring-1 ring-blue-500/20 hover:ring-blue-500/50 ${zenMode ? 'bg-zinc-900 border-zinc-800' : 'bg-zinc-900/50 border-blue-900/30'}`}>
                            ...
                            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0"><div className="relative flex h-4 w-4"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span><span className="relative inline-flex rounded-full h-4 w-4 bg-blue-500 border border-white/20"></span></div></div>
                            ...
                            <div className="relative z-10 mt-auto flex items-end justify-between w-full"><div className="text-xs font-mono text-zinc-400">{getSafeIp(selectedNode)}</div><div className="flex items-center gap-2 text-sm font-bold text-white"><span>{selectedNode.location?.countryName || 'Unknown'}</span><span className="text-lg">{getFlagEmoji(selectedNode.location?.countryCode)}</span></div></div>
                    ...
                    <div className="mt-auto pt-3 md:pt-6 border-t border-zinc-800 flex flex-col gap-2 md:gap-4">
                      <div className="flex flex-col items-center justify-center gap-3">
                        <div className="text-[10px] text-zinc-500 flex items-center gap-1.5 bg-black/40 px-3 py-1 rounded-full border border-zinc-800/50"><Clock size={10} /> Last Seen: <span className="text-zinc-300 font-mono">{timeAgo}</span></div>
                        <button onClick={() => copyToClipboard(`${window.location.origin}/?open=${selectedNode.pubkey}`, 'url')} className={`flex items-center justify-center gap-2 px-6 py-2 bg-blue-500/5 hover:bg-blue-500/10 border border-blue-500/20 rounded-full text-[10px] font-bold text-blue-400 transition`}>{copiedField === 'url' ? <Check size={12} /> : <LinkIcon size={12} />} {copiedField === 'url' ? 'LINK COPIED' : 'COPY NODE URL'}</button>
                      </div>
