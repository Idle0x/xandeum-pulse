...
import { 
  ...
  Globe, Clock, Database, ChevronRight 
} from 'lucide-react';
import { ChapterLayout } from '../layout/ChapterLayout';

// --- TEXT CONTENT ---
// Broken down for specific sections
const ENG_TEXT = [
    {
            ...
            <div className="flex flex-col gap-12 pb-8">
                ...
                <div className="flex flex-col gap-4">
                    <div className="prose prose-invert max-w-none">
                        ...
                        <p className="text-zinc-400 text-sm leading-relaxed mt-2">{ENG_TEXT[0].content}</p>
                    </div>
                ...
                <div className="flex flex-col gap-4">
                    <div className="prose prose-invert max-w-none">
                        ...
                        <p className="text-zinc-400 text-sm leading-relaxed mt-2">{ENG_TEXT[1].content}</p>
                    </div>
                ...
                <div className="flex flex-col gap-4">
                    <div className="prose prose-invert max-w-none">
                        ...
                        <p className="text-zinc-400 text-sm leading-relaxed mt-2">{ENG_TEXT[2].content}</p>
                    </div>
                ...
                <div className="flex flex-col gap-4">
                    <div className="prose prose-invert max-w-none">
                        ...
                        <p className="text-zinc-400 text-sm leading-relaxed mt-2">{ENG_TEXT[3].content}</p>
                    </div>
...
function PipelineMonitor({ diag }: { diag: DiagnosticState }) {
    ...
    return (
        <div className="bg-[#050505] border border-zinc-800 rounded-2xl p-6 relative overflow-hidden shadow-xl">
...
function WaterfallTrace({ diag }: { diag: DiagnosticState }) {
    ...
    return (
        <div className="bg-[#050505] border border-zinc-800 rounded-2xl p-6 relative overflow-hidden shadow-lg">
            <div className="flex justify-between items-end mb-4">
...
function DeduplicationLog({ count, status }: { count: number | undefined, status: string }) {
    ...
    const listRef = useRef<HTMLDivElement>(null); // Ref for the container, not a dummy div
    ...
    // FIXED: Scroll logic that targets the CONTAINER only
    useEffect(() => {
        if (listRef.current) {
    ...
    return (
        <div className="bg-black border border-zinc-800 rounded-2xl p-4 h-48 flex flex-col shadow-inner">
            ...
            {/* Attached Ref to the scrolling container */}
            <div ref={listRef} className="flex-1 overflow-y-auto font-mono text-[10px] space-y-1 text-zinc-400 custom-scrollbar scroll-smooth">
                {logs.map((log, i) => (
...
function MathTerminal({ node, status }: { node: any, status: string }) {
    if (status === 'IDLE' || status === 'DNS' || status === 'TCP' || status === 'SSL') {
        return (
            <div className="bg-[#050505] border border-zinc-800 rounded-2xl p-6 flex items-center justify-center text-zinc-600 text-[10px] uppercase tracking-widest h-full min-h-[240px] shadow-lg">
...
function ForensicFlags({ flags, status }: { flags: any, status: string }) {
    ...
    return (
        <div className="bg-[#050505] border border-zinc-800 rounded-2xl p-6 h-full flex flex-col justify-between shadow-lg min-h-[240px]">
