    ...
    return (
        <section className="max-w-5xl mx-auto px-6 py-24 relative">
            ...
            <div className="flex flex-col gap-16">
                ...
                {/* This section fades out in Zen Mode to focus purely on the data below */}
                <div className={`space-y-10 transition-opacity duration-1000 ${zenMode ? 'opacity-20 blur-sm pointer-events-none' : 'opacity-100'}`}>
                    ...
                    <div className="max-w-3xl">
                        ...
                        <h2 className="text-4xl md:text-5xl font-black text-white tracking-tighter mb-6">Telemetry & UX</h2>
                        <p className="text-zinc-400 leading-relaxed text-base md:text-lg">
                            The Pulse dashboard isn't just a data dump. It manages cognitive load and hardware longevity through adaptive layouts and preservation protocols.
                        </p>
                    </div>
                ...
                {/* --- 2. BOTTOM SECTION: The Simulation --- */}
                <div className={`flex flex-col gap-6 relative transition-all duration-1000 ${zenMode ? 'z-[50]' : 'z-0'}`}>
                    
                    {/* SIMULATION CONTAINER */}
                    <div className={`
                        w-full rounded-3xl overflow-hidden border transition-all duration-1000 relative h-[400px] flex flex-col
                        ${zenMode ? 'bg-black border-zinc-900 shadow-none' : 'bg-[#09090b] border-zinc-800 shadow-2xl'}
                    `}>
                        
                        {/* Control Deck */}
                        <div className={`
                            flex items-center justify-between px-6 py-4 border-b transition-colors duration-1000 shrink-0
                        ...
                        `}>
                            ...
                            <button 
                                ...
                                className={`
                                    group relative pl-3 pr-4 py-2 rounded-full border text-[10px] font-bold uppercase tracking-widest transition-all duration-500 flex items-center gap-2
                                    ${zenMode 
                                    ...
                                    {[1, 2, 3, 4, 5, 6].map((i) => (
                                        <div key={i} className={`flex items-center justify-between p-3 rounded-lg border transition-colors ${zenMode ? 'border-zinc-900 text-zinc-600' : 'bg-zinc-900/20 border-zinc-800 hover:bg-zinc-900 text-zinc-400'}`}>
                                            <div className="flex items-center gap-3">
                                                <div className={`w-1.5 h-1.5 rounded-full ${zenMode ? 'bg-zinc-800' : 'bg-blue-500'}`}></div>
                                                <span className="text-xs font-mono">0x82...9A</span>
                                            </div>
                                            <span className="text-[10px] uppercase font-bold text-zinc-600 hidden sm:block">Tokyo</span>
                                            <div className="flex gap-4">
                                                <span className={`text-xs font-mono w-10 text-right ${zenMode ? 'text-zinc-700' : 'text-green-500'}`}>98%</span>
                                                <span className="text-xs font-mono w-12 text-right">1.2PB</span>
                                            </div>
                                        </div>
                                    ))}
                                </div>
