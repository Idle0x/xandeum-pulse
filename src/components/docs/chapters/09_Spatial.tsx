...
const SPATIAL_LOGIC_SNIPPET = `
const resolveKingNode = (cityNodes, activeMetric) => {
  // Sorts nodes in a city cluster by performance to find the "King"
  // Tournament Logic: The leader represents the geographic density of that shard
  return cityNodes.reduce((prev, current) => {
    const pVal = prev[activeMetric] || 0;
    const cVal = current[activeMetric] || 0;
    return (cVal > pVal) ? current : prev;
  });
};
`;
...
export function SpatialChapter() {
    return (
        <LogicWrapper 
            title="Spatial_Topology.ts" 
            code={SPATIAL_LOGIC_SNIPPET} 
            githubPath="src/logic/spatial-resolution.ts"
        >
                ...
                {selectedCity && currentCityData ? (
                    <div className="flex-1 flex flex-col p-6 animate-in slide-in-from-bottom duration-700">
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-8">
                             <div className="p-4 bg-zinc-900/50 rounded-2xl border border-zinc-800 flex flex-col justify-center text-left">
                        ...
                        <div className="mt-auto bg-zinc-900/80 border border-cyan-500/30 rounded-3xl p-6 relative overflow-hidden group text-left">
