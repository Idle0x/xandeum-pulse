                    ...
                    {MODULES.map((mod) => (
                        <button
                            ...
                            className={`
                                ...
                                ${activeTab === mod.id ? 'text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}
                            `}
                        >
                            {/* Active Tab Background */}
                            {activeTab === mod.id && (
                                <div className="absolute inset-0 bg-zinc-800 rounded-full border border-zinc-700 shadow-inner -z-10"></div>
                            )}
                            <mod.icon size={12} className={activeTab === mod.id ? 'text-blue-400' : 'text-zinc-600'} />
                            {mod.label}
                        </button>
                    ))}
                </div>
                ...
                <div className={`transition-all duration-1000 delay-500 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                    ...
                    >
                        ...
                        <span className="relative flex items-center gap-3">
                            Open {activeModule.label} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform"/>
                        </span>
                    </button>
                </div>
            </div>
