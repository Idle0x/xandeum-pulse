  ...
  return useMemo((): VitalityResult => {
    ...
    if (sortedHistory.length > 1) {
        for (let i = 0; i < sortedHistory.length - 1; i++) {
            ...
            // Logic: Uptime dropped significantly AND is currently low (< 1h)
    ...
    // LEVEL 4: THE "NEW NODE" CHECK (WARMUP)
    // FIX: Based on Node Age (Database Record), NOT Uptime.
    // Logic: If node was created in DB less than 3 days ago.
    const createdAt = node.created_at ? new Date(node.created_at).getTime() : 0; 
    const nodeAgeMs = now - createdAt;
    // 3 Days = 259,200,000 ms
    // We only check this if createdAt is valid (> 1970)
    if (createdAt > 0 && nodeAgeMs < 259200000) {
        return {
            status: 'WARMUP',
            label: 'WARMING UP',
            color: 'text-blue-400',
            ...
            icon: ThermometerSun,
            // 
            reason: `Incubation Period: Active for ${Math.round(nodeAgeMs / (1000 * 60 * 60 * 24))} days`,
            confidence: 100
        };
    }

    // LEVEL 5: THE "SURVIVOR" CHECK (ONLINE)
    // An old node that just restarted (uptime < 30m) falls through to here.
    return {
        status: 'ONLINE',
        label: 'ONLINE',
