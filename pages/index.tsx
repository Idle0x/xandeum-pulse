...
export default function MapPage() {
  ...
  // --- STICKY FOCUS: AUTO-SCROLL ON MODE CHANGE ---
  useEffect(() => {
      // Whenever viewMode changes OR we select a location, ensure it stays in view
      if (activeLocation && isSplitView) {
          // Small timeout to allow the DOM list to re-sort
          setTimeout(() => {
  ...
  const lockTarget = (name: string, lat: number, lon: number) => {
    // Standard locking logic (updates state, which triggers the useEffect above)
