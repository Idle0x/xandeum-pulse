...
export default function Home() {
  ...
  const renderHealthBreakdown = () => {
    ...
    const rankPercentile = (rank / totalNodes) * 100; // Where you stand
    const betterThanPercent = 100 - rankPercentile; // Better than X%
...
>
  ...
  <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity text-[8px] text-blue-400 font-bold flex items-center gap-1 z-10">
    <Maximize2 size={8} /> DETAILS
            ...
            <div 
  onClick={() => {
    // Cycle through filters: ALL → MAINNET → DEVNET → ALL
...
<div className="flex justify-between">
  <span className="text-zinc-500 text-xs">Network Average</span>
  <span className="text-zinc-300 font-mono text-sm">
    {formatBytes(totalStorageCommitted / (nodes.length || 1))}
  </span>
</div>
                              </div>
                              <div className="mt-6 text-center w-full z-10 flex justify-center">
