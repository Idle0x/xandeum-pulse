...
export default function Home() {
  ...
  const handleSortChange = (metric: 'uptime' | 'version' | 'storage' | 'health') => {
      // 1. Toggle Order if same metric, else set new metric desc
      if (sortBy === metric) {
      ...
      // 2. FORCE CYCLE TO SHOW THIS METRIC
      let targetStep = 1; // Default to Storage
      if (metric === 'health') targetStep = 2;
      if (metric === 'uptime') targetStep = 3;
      
      setCycleStep(targetStep);
      setCycleReset(prev => prev + 1); // Resets the timer
  };
  ...
  const renderNodeCard = (node: Node, i: number) => {
    ...
    return (
      <div
        key={`${node.pubkey}-${node.network}-${i}`} // Updated Key
      ...
      >
        ...
        <div className="space-y-1.5 md:space-y-3">
          <div className="flex justify-between items-center text-[10px] md:text-xs">
            ...
            <span className={`px-2 py-0.5 rounded transition-all duration-500 ${isVersionSort ? 'text-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.4)] border-cyan-500/50 bg-zinc-900 border' : 'text-zinc-300 bg-zinc-800'}`}>
  ...
  // ... [Keep renderZenCard, renderIdentityDetails, renderHealthBreakdown, renderStorageAnalysis EXACTLY as they were] ...
  // (Assuming you have these blocks already, I am not re-pasting them to save space, they are unchanged except for internal logic usage)
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 ${zenMode ? 'bg-black text-zinc-300 selection:bg-zinc-700' : 'bg-[#09090b] text-zinc-100 selection:bg-blue-500/30'}`} onClick={handleGlobalClick}>
      ...
      <div className={`fixed inset-y-0 left-0 w-72 bg-[#09090b] border-r border-zinc-800 z-[200] transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex flex-col h-full">
          <div className="flex justify-between items-center mb-8">
            ...
            <button onClick={() => setIsMenuOpen(false)} className="text-zinc-500 hover:text-white p-2 rounded-lg bg-zinc-900 border border-zinc-800"><X size={24} /></button>
          </div>
      ...
      <header className={`sticky top-0 z-[100] backdrop-blur-md border-b px-6 py-2 md:py-4 flex flex-col gap-4 md:gap-6 transition-all duration-500 ${zenMode ? 'bg-black/90 border-zinc-800' : 'bg-[#09090b]/90 border-zinc-800'}`}>
        <div className="flex justify-between items-center w-full">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsMenuOpen(true)} className={`p-2.5 md:p-3.5 rounded-xl transition ${zenMode ? 'text-zinc-400 border border-zinc-800' : 'text-zinc-400 bg-zinc-900 border border-zinc-700 hover:text-white hover:bg-zinc-800'}`}><Menu size={24} className="md:w-7 md:h-7" /></button>
            <div className="flex flex-col">
              <h1 className={`text-lg md:text-xl font-extrabold tracking-tight flex items-center gap-2 ${zenMode ? 'text-white' : 'text-white'}`}><Activity className={zenMode ? 'text-zinc-500' : 'text-blue-500'} size={20} /> PULSE</h1>
      ...
      {toast && toast.visible && <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[200] animate-in fade-in slide-in-from-top-4 duration-300 w-full max-w-md px-4 pointer-events-none"><div className="bg-zinc-900 border border-yellow-500/30 text-zinc-200 px-4 py-3 rounded-xl shadow-2xl flex items-start gap-3 pointer-events-auto"><AlertTriangle size={20} className="text-yellow-500 shrink-0 mt-0.5" /><div className="text-xs font-bold leading-relaxed">{toast.msg}</div><button onClick={() => setToast(null)} className="text-zinc-500 hover:text-white ml-auto"><X size={16}/></button></div></div>}
      ...
      <main className={`p-4 md:p-8 ${zenMode ? 'max-w-full' : 'max-w-7xl 2xl:max-w-[1800px] mx-auto'} transition-all duration-500`}>
        {!zenMode && !loading && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 mb-4 md:mb-8">
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 p-3 md:p-5 rounded-xl backdrop-blur-sm"><div className="text-[8px] md:text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Total Nodes</div><div className="text-lg md:text-3xl font-bold text-white mt-1">{nodes.length}</div></div>
          </div>
        )}
        ...
        {!loading && nodes.length > 0 && <div className="flex items-center gap-2 mb-4 mt-8"><Activity className="text-green-500" size={20} /><h3 className="text-lg font-bold text-white tracking-widest uppercase">Total Nodes - {filteredNodes.length}</h3><div className="flex flex-col justify-center ml-2 leading-none"><span className="text-[7px] md:text-[9px] font-mono text-zinc-500 uppercase">(Distributed by <span className="text-zinc-300">{sortBy}</span></span><span className="text-[7px] md:text-[9px] font-mono text-zinc-500 uppercase text-center">{sortOrder === 'asc' ? 'Lowest to Highest' : 'Highest to Lowest'})</span></div></div>}

        {loading && nodes.length === 0 ? <PulseGraphLoader /> : <div className={`grid gap-4 ${zenMode ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:gap-8'} pb-20`}>{filteredNodes.map((node, i) => { if (zenMode) return <div key={`${node.pubkey}-${node.network}-${i}`}>{renderZenCard(node)}</div>; return renderNodeCard(node, i); })}</div>}
      </main>
            ...
            <div className={`shrink-0 p-4 md:p-6 border-b flex justify-between items-start ${zenMode ? 'bg-black border-zinc-800' : 'bg-zinc-900/50 border-zinc-800'}`}>
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-zinc-500 font-mono mt-1"><span className="text-zinc-400 truncate max-w-[120px] md:max-w-none">{selectedNode.pubkey ? `${selectedNode.pubkey.slice(0, 12)}...` : 'Unknown'}</span><Copy size={10} className="cursor-pointer hover:text-white" onClick={() => copyToClipboard(selectedNode.pubkey || '', 'pubkey')} /></div>
