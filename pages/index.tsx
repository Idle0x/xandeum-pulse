...
interface Node {
  ...
  health?: number;
  // UPDATED: 'capacity' renamed to 'storage' to match the new Brain logic
  healthBreakdown?: {
      uptime: number;
      version: number;
      reputation: number;
...
export default function Home() {
  ...
  // UPDATED: Using 'storage' instead of 'capacity' in defaults
  ...
  const renderHealthBreakdown = () => {
      ...
      // UPDATED: Metric labels and keys match the new logic
      const metrics = [
          ...
          <div className="animate-in fade-in slide-in-from-right-2 duration-200 h-full flex flex-col">
              ...
              <div className="flex-grow flex flex-col gap-6">
                  ...
                  <div className="p-6 bg-black rounded-2xl border border-zinc-800 flex justify-between items-center relative overflow-hidden">
                      ...
                      <div className="text-right">
                          ...
                          <div className="flex items-center gap-2 justify-end">
                              ...
                              <span className={`text-xs font-bold px-1.5 py-0.5 rounded ${diff >= 0 ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
                                  {diff > 0 ? '+' : ''}{diff}
                              </span>
                          </div>
                      </div>
                  ...
                  <div className="space-y-5">
                      {metrics.map((m) => {
                          const barColor = m.val >= 80 ? 'bg-green-500' : m.val >= 50 ? 'bg-yellow-500' : 'bg-red-500';
                          return (
                              <div key={m.label}>
                                  ...
                                  <div className="h-2 bg-zinc-800 rounded-full overflow-visible relative">
                                      <div className={`h-full rounded-l-full transition-all duration-1000 ${barColor} shadow-[0_0_10px_rgba(255,255,255,0.1)]`} style={{ width: `${m.val}%` }}></div>
                                      <div 
  ...
  return (
    <div className={`min-h-screen font-sans transition-colors duration-500 ${zenMode ? 'bg-black text-zinc-300 selection:bg-zinc-700' : 'bg-[#09090b] text-zinc-100 selection:bg-blue-500/30'}`} onClick={handleGlobalClick}>
      ...
      {!loading && (
         <div className={`sticky top-[208px] md:top-[212px] z-[90] flex flex-col transition-all duration-300 ${scrolled ? 'shadow-xl' : ''}`}>
             <div className={`w-full border-b border-zinc-800/50 py-3 px-6 flex items-center justify-center backdrop-blur-md transition-colors duration-300 ${scrolled ? 'bg-black/95 border-b-zinc-800' : 'bg-[#09090b]/80'}`}>
                 <div className="text-xs md:text-sm font-bold font-mono text-zinc-400 uppercase tracking-widest flex items-center gap-2"><Activity size={14} className={scrolled ? "text-blue-500" : "text-zinc-600"}/>Nodes distributed by <span className="text-white">{sortBy.toUpperCase()}</span> <span className="text-zinc-600">({sortOrder === 'desc' ? 'High to Low' : 'Low to High'})</span></div>
             </div>
             {searchQuery && (<div className="w-full bg-blue-900/90 border-b border-blue-500/30 py-2 px-6 text-center backdrop-blur-md animate-in slide-in-from-top-1"><div className="text-xs font-mono text-blue-100">Found <span className="font-bold text-white">{filteredNodes.length}</span> matches for <span className="italic">"{searchQuery}"</span></div></div>)}
         </div>
      )}
