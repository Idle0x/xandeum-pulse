...
function PulseOS_Simulator() {
    ...
    useEffect(() => {
        if (view !== 'BOOT') return; // Guard clause
                            ...
                            {[1,2,3,4,5].map(i => (
                                <div key={i} 
                                    onClick={() => {
                                        ...
                                        if (i === 1 && readyButtons.includes('card-1')) {
                                            navigate('MODAL', 800);
                                        }
                                    }}
                                    className={`h-40 md:h-48 border rounded-xl p-4 flex flex-col justify-between transition-all duration-300 relative ${i===1 && readyButtons.includes('card-1') ? 'border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.5)] bg-zinc-900 cursor-pointer scale-105' : 'border-zinc-800 bg-zinc-900/30 opacity-40'}`}
                                >
